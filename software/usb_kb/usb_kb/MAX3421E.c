...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	//write val via SPI
	alt_u8 write_data[2] = {reg + 2, val};
	//read return code from SPI peripheral (see Intel documentation)
	int ret = alt_avalon_spi_command(SPI_0_BASE, 0x0, 2, write_data, 0, NULL, 0);
	//if return code < 0 print an error
	if (ret < 0) {
		printf("You're a dumbass, but you already knew that right?");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	//write reg + 2 via SPI (0th element)
	alt_u8 write_data[nbytes + 1];
	write_data[0] = reg + 2;
	//write data[n] via SPI, where n goes from 0 to nbytes-1
	for (int i = 1; i < nbytes + 1; i++) {write_data[i] = data[i - 1];}
	//read return code from SPI peripheral (see Intel documentation)
	int ret = alt_avalon_spi_command(SPI_0_BASE, 0x0, nbytes + 1, write_data, 0, NULL, 0);
	//if return code < 0  print an error
	if (ret < 0) {
		printf("You're a dumbass, but you already knew that right?");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	//write reg via SPI (write_length = 1)
	//read val via SPI (read_length = 1)
	alt_u8 val;
	//read return code from SPI peripheral (see Intel documentation)
	int ret = alt_avalon_spi_command(SPI_0_BASE, 0x0, 1, &reg, 1, &val, 0);
	//if return code < 0 print an error
	if (ret < 0) {
		printf("You're a dumbass, but you already knew that right?");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//read return code from SPI peripheral (see Intel documentation)
	int ret = alt_avalon_spi_command(SPI_0_BASE, 0x0, 1, &reg, nbytes, data, 0);
	//if return code < 0 print an error
	if (ret < 0) {
		printf("You're a dumbass, but you already knew that right?");
	}
